...
const Dashboard: React.FC = () => {
  ...
  const [managingSchedule, setManagingSchedule] = useState<DayPlan | null>(null);
  ...
  const handleCreateEvent = (eventData: Omit<Event, 'id' | 'createdAt' | 'updatedAt' | 'dayPlans'>) => {
  ...
  const handleUpdateEvent = (eventData: Omit<Event, 'id' | 'createdAt' | 'updatedAt' | 'dayPlans'>) => {
  ...
  const handleDeleteEvent = (eventId: string) => {
    if (confirm('Möchten Sie diese Veranstaltung wirklich löschen? Alle Tagespläne werden ebenfalls gelöscht.')) {
  ...
  const handleCreateDayPlan = (dayPlanData: Omit<DayPlan, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const handleUpdateDayPlan = (dayPlanData: Omit<DayPlan, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const handleDeleteDayPlan = (dayPlanId: string) => {
    ...
    if (confirm('Möchten Sie diesen Tagesplan wirklich löschen?')) {
      const updatedEvents = events.map(e => 
        e.id === selectedEvent.id 
          ? { ...e, dayPlans: e.dayPlans.filter(d => d.id !== dayPlanId), updatedAt: new Date() }
          : e
      );
      saveEvents(updatedEvents);
      setSelectedEvent(updatedEvents.find(e => e.id === selectedEvent.id) || null);
            ...
            {events.length === 0 ? (
              ...
              events.map(event => (
                <div
                  ...
                  style={{
                    padding: '1rem',
                    border: `2px solid ${selectedEvent?.id === event.id ? '#fbbf24' : '#e5e7eb'}`,
                    borderRadius: '8px',
                    backgroundColor: selectedEvent?.id === event.id ? '#fef3c7' : '#fff',
                    cursor: 'pointer',
                    transition: 'all 0.2s ease',
                    boxShadow: selectedEvent?.id === event.id ? '2px 4px 0 #fbbf24' : 'none'
                  }}
                >
                  <div style={{ 
                    display: 'flex', 
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ 
                        ...
                        <FileText size={12} />
                        {event.dayPlans.length} Tagesplan{event.dayPlans.length !== 1 ? 'e' : ''}
                      </div>
                    </div>
                      ...
                      <div style={{ 
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ 
                            ...
                            <Calendar size={12} />
                            {dayPlan.schedule.length} Termin{dayPlan.schedule.length !== 1 ? 'e' : ''}
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                          ...
                          <button
                            onClick={() => {
                              // Preview functionality - could navigate to planner view
                              alert('Vorschau-Funktion: Hier könnte zur Planer-Ansicht navigiert werden');
                            }}
                            style={{
                              padding: '0.4rem',
                              border: 'none',
